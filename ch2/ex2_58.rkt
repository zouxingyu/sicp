...
(define (same-variable? e1 e2) (and (variable? e1) (variable? e2) (eq? e1 e2)))
...
(define (add1 e)
  (define (iter r expr)
    (if (eq? '+ (car expr)) r (iter (append r (list (car expr))) (cdr expr))))
    (let ((result (iter '() e)))
      (if (= (length result) 1) (car result) result)))
(define (add2 e)
  (let ((result (cdr (memq '+ e))))
    (if (= (length result) 1) (car result) result)))

(define (product? e) (if (memq '* e) #t #f))
 ...
 (define (multi2 expr) 
   (let ((result (cdr (memq '* expr)))) 
     (if (= (length result) 1) 
         (car result) 
         result))) 
(define (expo? e)  (and (pair? e) (eq? (cadr e) '**)))
